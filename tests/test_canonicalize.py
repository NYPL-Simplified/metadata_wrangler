...
class TestAuthorNameCanonicalizer(DatabaseTest):
    ...
    def setup_method(self):
        super(TestAuthorNameCanonicalizer, self).setup_method()
    ...
    def test_primary_author_name(self):
        ...
        assert ("Vassar College and its Board of Directors" ==
            m("Vassar College and its Board of Directors"))

        # Test the simplest case -- one human name.
        assert "Mindy Kaling" == m("Mindy Kaling")

        # When there are multiple humans, only the first one is used.
        assert "Mindy Kaling" == m("Mindy Kaling, Bob Saget and Co")
        assert "Bill O'Reilly" == m("Bill O'Reilly with Martin Dugard")
        assert ("Clare Verbeek" ==
    ...
    def test_canonicalize_author_name(self):
        ...
        with pytest.raises(CanonicalizationError) as excinfo:
            self.canonicalizer.canonicalize_author_name(None, '')
        assert "Neither useful identifier nor display name was provided." in str(excinfo.value)
        ...
        # default_name.
        assert ("Default Jim Davis" ==
            c.canonicalize_author_name("Jim Davis", "An ISBN"))
        assert [("Jim Davis", "An ISBN")] == c.attempts
        c.attempts = []
        ...
        # string.
        assert ("Default Jim Davis and Matt Groening" ==
    ...
    def test_sort_name_from_services(self):
        ...
        assert "good value from database" == m(*args)

        del c.return_values['sort_name_from_database']
        assert "good value from OCLC" == m(*args)

        del c.return_values['sort_name_from_oclc_linked_data']
        assert "good value from VIAF URLs" == m(*args)

        del c.return_values['sort_name_from_viaf_urls']
        assert "good value from VIAF display name" == m(*args)
    ...
    def test_sort_name_from_database(self):
        ...
        assert "Sort Name, The Real" == answer

        # It also returned all titles associated with the identifier
        # we passed in.
        assert set(["Title 1", "Title 2"]) == titles
        ...
        assert (c1, titles) == call1
        assert (c2, titles) == call2
    ...
    def test__sort_name_from_contributor_and_titles(self):
        ...
        assert None == m(None, None)
        assert None == m(None, ["Title 1", "Title 2"])

        # This contributor has no contributions at all -> failure
        no_contributions, ignore = self._contributor()
        assert None == m(no_contributions, ["Title 1"])
        ...
        # substantial match.
        assert None == m(contributor, None)
        assert None == m(contributor, [])
        assert None == m(contributor, ["Title 1", "Title 2"])
    ...
    def test_sort_name_from_viaf_urls(self):
        ...
        assert None == m("Jim Davis", None)
        assert (
            None ==
            m("Jim Davis", ["http://not-a-viaf/", "http://still-not-a-viaf/"])
        )
        ...
        assert [] == self.viaf_client.viaf_lookups
        ((viaf_id,), kwargs) = lookup
        assert "1234" == viaf_id
        assert dict(working_display_name="Jim Davis") == kwargs

        # We got the right answer.
        assert "Davis, Jim (from VIAF)" == result
    ...
    def test_sort_name_from_viaf_display_name(self):
        ...
        args, kwargs = lookup
        assert (dict(known_titles=['Garfield Hates Mondays'],
    ...
    def test_default_sort_name(self):
        ...
        assert "Davis, Jim" == m("Jim Davis")
        assert "Davis, Jim" == m("Jim Davis and Matt Groening")
        assert "Vassar College" == m("Vassar College")
