...
dc_ns = 'http://dublincore.com/'

nsmap = {'': atom_ns, 'app': app_ns, 'dc' : dc_ns}
...
class AtomLookup(etree.CustomElementClassLookup):
    ...
    def lookup(self, node_type, document, namespace, name):
        if node_type == 'element':
            ...
            elif namespace == dc_ns:
                return self._dc_elements.get(name, DCElement)
...
"""
    ...
    def __init__(self, tag, ns=atom_ns, strip=True):
        self.tag = tag
        self.ns = ns
        self.strip = strip
...
class DCElement(etree.ElementBase):
    def __repr__(self):
        tag = self.tag
        if '}' in tag:
            tag = tag.split('}', 1)[1]
        return '<%s.%s dc:%s at %s>' % (
            self.__class__.__module__,
            self.__class__.__name__,
            tag,
            hex(abs(id(self)))[2:])

class Language(DCElement):
    pass


AtomLookup._dc_elements.update(dict(
    language=Language,
))
